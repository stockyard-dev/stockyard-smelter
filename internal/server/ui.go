...
async function load(){var r=await fetch(A+'/transforms').then(function(r){return r.json()});items=r.transforms||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.input_format||'').toLowerCase().includes(q)||(i.output_format||'').toLowerCase().includes(q)||(i.template||'').toLowerCase().includes(q)});
...
if(i.input_format)h+='<div class="item-sub">'+esc(i.input_format)+'</div>';
...
if(i.output_format)h+='<span>'+esc(i.output_format)+'</span>';
if(i.template)h+='<span>'+esc(i.template)+'</span>';
if(i.run_count)h+='<span>'+esc(i.run_count)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/transforms/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",input_format:"",output_format:"",template:"",description:"",run_count:"",last_run_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SMELTER</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Input Format</label><input id="f-input-format" value="'+esc(i.input_format)+'"></div>';
h+='<div class="fr"><label>Output Format</label><input id="f-output-format" value="'+esc(i.output_format)+'"></div>';
h+='<div class="fr"><label>Template</label><input id="f-template" value="'+esc(i.template)+'"></div>';
h+='<div class="fr"><label>Description</label><textarea id="f-description" rows="3">'+esc(i.description)+'</textarea></div>';
h+='<div class="fr"><label>Run Count</label><input id="f-run-count" value="'+esc(i.run_count)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),input_format:document.getElementById('f-input-format').value.trim(),output_format:document.getElementById('f-output-format').value.trim(),template:document.getElementById('f-template').value.trim(),description:document.getElementById('f-description').value.trim(),run_count:document.getElementById('f-run-count').value.trim(),last_run_at:document.getElementById('f-last-run-at').value.trim()};
if(editId){await fetch(A+'/transforms/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/transforms',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
